...
export default function StepThree({ formData, onUpdateFormData }: StepProps) {
  ...
  const handleServiceToggle = (uiId: string) => {
    ...
    // if (newSelectedUiIds.length === 1 && newSelectedUiIds[0] === "DEAL: Inspection") {
    //   apiId = "repair"
    // }
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
        {serviceTypes.map((service) => {
          ...
          return (
            ...
            >
              {isSelected && (
                <div className="absolute top-3 right-4 w-6 h-6 rounded-full flex items-center justify-center bg-green-500">
