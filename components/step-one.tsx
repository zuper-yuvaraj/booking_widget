...
import { COMPANY_UUID, SERVICES_WEBHOOK } from "@/configs"
  ...
  const handleAddressSelect = (prediction: GoogleMapsPrediction) => {
    ...
    placesService.getDetails(request, (place: any, status: string) => {
      if (status === window.google.maps.places.PlacesServiceStatus.OK) {
        ...
        fetch(`${SERVICES_WEBHOOK}?${params.toString()}`)
          .then((res) => res.json())
          .then((data) => {
    ...
    <div className="space-y-6">
      ...
      <div className="relative">
        <input
          ...
          className="w-full px-4 py-3 border border-primary/40 rounded-lg shadow-sm text-lg focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary/25 caret-[#ed1b24]"
          disabled={!isLoaded}
        />
          ...
          <div className="absolute z-10 w-full mt-1 bg-white border border-gray-300 rounded-lg shadow-lg max-h-60 overflow-y-auto">
            {predictions.map((prediction) => (
              <div
                ...
                className="px-4 py-3 cursor-pointer border-b last:border-b-0 border-gray-100 hover:bg-primary/10 transition-colors"
              >
      ...
      {formData.address && (
        <div className={`p-4 rounded-lg border ${formData.serviceAreaMessage && !formData.isServiceAreaValid ? "bg-red-50 border-red-200" : "bg-primary/10 border-primary/30"}`}>
          {!isCheckingService && formData.serviceAreaMessage && (
            <p className={`text-sm font-medium mb-2 ${formData.isServiceAreaValid ? "text-primary" : "text-red-700"}`}>
